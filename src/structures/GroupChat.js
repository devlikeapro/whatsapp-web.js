        ...
        return await this.client.pupPage.evaluate(
            async (groupId, participantIds, options) => {
                ...
                for (let pWid of participantWids) {
                    const pId = window.WWebJS.GetSerialized(pWid);
                    pWid =
                        pWid.server === 'lid'
                            ? window
                    ...
                    if (
                        groupParticipants.some(
                            (p) => window.WWebJS.GetSerialized(p) === pId,
                        )
                    ) {
                        ...
                        if (
                            ...
                            const res = await window
                                ...
                                .sendGroupInviteMessage(
                                    userChat,
                                    window.WWebJS.GetSerialized(group.id),
                                    groupName,
                        ...
                        participantIds.map(async (p) => {
                            ...
                            return (
                                chat.groupMetadata.participants.get(
                                    window.WWebJS.GetSerialized(lid),
                                ) ||
                                chat.groupMetadata.participants.get(
                                    window.WWebJS.GetSerialized(phone),
                                )
                            );
                        }),
                    )
